% F3: Optimize Rastrigin's Function using GA, PSO, and SA
clc;
clear;
...
% Run optimization 15 times for Rastrigin function
for i = 1:15
    % GA Optimization
    options_ga = optimoptions('ga', 'MaxGenerations', 100, 'PopulationSize', 50, 'Display', 'off');
    [~, ga_results(i)] = ga(@rastrigin, nvars, [], [], [], [], lb, ub, [], options_ga);

    % PSO Optimization
    options_pso = optimoptions('particleswarm', 'SwarmSize', 50, 'MaxIterations', 100, 'Display', 'off');
    [~, pso_results(i)] = particleswarm(@rastrigin, nvars, lb, ub, options_pso);

    % SA Optimization
    options_sa = optimoptions('simulannealbnd', 'MaxIterations', 100, 'Display', 'off');
    [~, sa_results(i)] = simulannealbnd(@rastrigin, rand(1, nvars), lb, ub, options_sa);
end
...
% Display results
disp(['F3 (Rastrigin) - GA Best: ', num2str(ga_best), ', Avg: ', num2str(ga_avg), ', Std: ', num2str(ga_std)]);
disp(['F3 (Rastrigin) - PSO Best: ', num2str(pso_best), ', Avg: ', num2str(pso_avg), ', Std: ', num2str(pso_std)]);
disp(['F3 (Rastrigin) - SA Best: ', num2str(sa_best), ', Avg: ', num2str(sa_avg), ', Std: ', num2str(sa_std)]);